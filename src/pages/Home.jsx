...
import { ArrowRight, Store, ShieldCheck, BadgeCheck, TrendingUp, Package, Heart, Sparkles } from "lucide-react";
...
// Rank a listing by how "featured" it is: explicit featured flag first,
// then a blend of views/likes/favorites so the home page surfaces the most
// popular assets even when nothing is hand-picked.
function rankScore(l) {
  return (l.featured ? 1000 : 0) + (l.views || 0) + (l.likes || 0) + (l.favorites || 0) + (l.favorite_count || 0);
}
...
export default function Home() {
  ...
  useEffect(() => {
    db.entities.Listing.filter({ status: "active" }, "-created_date", 12)
      .then((rows) => setListings(rows.slice().sort((a, b) => rankScore(b) - rankScore(a)).slice(0, 4)))
      .catch(() => {})
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-[-12%] left-[18%] w-[620px] h-[420px] bg-primary/[0.12] rounded-full blur-[150px]" />
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_0%,hsl(168_60%_50%/0.08),transparent_55%)]" />
        </div>
          ...
          <div className="grid lg:grid-cols-[1.1fr_1fr] gap-12 lg:gap-16 items-center">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-6">
                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/25 text-primary text-xs font-medium">
              ...
              <h1 className="font-display text-4xl lg:text-6xl font-bold tracking-tight leading-[1.08] mb-5">
                The trusted marketplace for{" "}
                <span className="text-primary text-glow">ER:LC</span> creators.
              </h1>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Link to="/marketplace" className="inline-flex items-center gap-2 bg-primary hover:opacity-90 text-primary-foreground font-semibold px-6 py-3 rounded-xl transition text-base glow-primary">
            ...
            {/* Right: Featured today */}
            <div className="relative">
              <div className="rounded-2xl glass border border-border p-4 glow-primary">
                <div className="flex items-center justify-between mb-3 px-1">
                  <span className="label-mono text-primary flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" /> Featured today</span>
                  <span className="text-xs text-primary font-medium flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" /> Live</span>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {!loading && listings.length === 0 && (
                    Array.from({ length: 4 }).map((_, i) => (
                      <div key={i} className="aspect-[4/3] rounded-lg bg-secondary/50 grid place-items-center border border-border">
              ...
              <div className="absolute -bottom-3 -left-3 rounded-xl glass border border-border px-4 py-2.5 hidden sm:flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-semibold text-foreground leading-none">Top 4 picks</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">refreshed daily</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-5 lg:px-8 py-16">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="label-mono text-primary mb-1.5">Departments</p>
            <h2 className="font-display text-2xl lg:text-3xl font-bold tracking-tight">Shop by department</h2>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {DEPARTMENTS.map((d) => (
            <Link
              ...
              className="group rounded-xl bg-card border border-border p-5 hover:border-primary/40 hover:bg-card/60 transition"
            >
              <div className="w-14 h-14 mb-4 rounded-lg overflow-hidden bg-secondary/50 ring-1 ring-border grid place-items-center">
                <Image src={d.logo} alt={d.name} fittingType="contain" className="w-full h-full object-contain p-2" />
              </div>
      ...
      <section className="max-w-7xl mx-auto px-5 lg:px-8 py-16 border-t border-border">
        <div className="flex items-end justify-between mb-8">
          <div>
            <p className="label-mono text-primary mb-1.5">Featured</p>
            <h2 className="font-display text-2xl lg:text-3xl font-bold tracking-tight">Top listings right now</h2>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <div key={s.l} className="rounded-xl bg-card border border-border p-6">
              <div className="font-display text-3xl lg:text-4xl font-bold text-primary text-glow">{s.v}</div>
      ...
      <section className="max-w-7xl mx-auto px-5 lg:px-8 py-16 pb-24">
        <div className="relative rounded-2xl overflow-hidden glass border border-primary/25 p-10 lg:p-16 glow-primary">
          <div className="absolute inset-0 grid-bg opacity-40 pointer-events-none" />
          <div className="relative text-center">
            <h2 className="font-display text-2xl lg:text-4xl font-bold tracking-tight mb-3">Ready to list your assets?</h2>
            <p className="text-muted-foreground max-w-xl mx-auto mb-7">Create a listing in minutes. Free or paid in Robux — your choice.</p>
            <Link to="/sell" className="inline-flex items-center gap-2 bg-primary hover:opacity-90 text-primary-foreground font-semibold px-7 py-3.5 rounded-xl transition glow-primary">
...
function FeaturedCard({ listing }) {
  ...
  return (
    <Link to={`/listing/${listing.id}`} className="group block rounded-lg overflow-hidden bg-secondary/40 border border-border hover:border-primary/40 transition">
...
export function ListingCard({ listing }) {
  ...
  return (
    <Link to={`/listing/${listing.id}`} className="group block rounded-xl overflow-hidden bg-card border border-border hover:border-primary/40 hover:bg-card/60 transition">
      <div className="aspect-[4/3] bg-secondary overflow-hidden relative">
        {cover ? (
    ...
    <footer className="border-t border-border">
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-12 grid grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        {[
          ...
          { h: "Account", links: [["Sign in", "/login"], ["Dashboard", "/dashboard"]] },
        ].map((col) => (
          <div key={col.h}>
